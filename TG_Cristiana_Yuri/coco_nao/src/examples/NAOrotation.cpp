...
#include "../controllers/RotationController.hpp"
#include "../robot_dh/NAO.h" //Has NAOKinemetics_LArm()
#include "NAOrotation.hpp"
...
cocoNAO::cocoNAO()
{
    ...
    //Results.open("/home/cris/catkin_ws/src/coco_nao/resultados_translation_controller2.txt");

    fileIndexPosL = Save.createFile("/home/cris/catkin_ws/src/coco_nao/joints_rotation_left_6.txt");
    fileIndexErrorL = Save.createFile("/home/cris/catkin_ws/src/coco_nao/error_rotation_left_6.txt");

    fileIndexPosR = Save.createFile("/home/cris/catkin_ws/src/coco_nao/joints_rotation_right_6.txt");
    fileIndexErrorR = Save.createFile("/home/cris/catkin_ws/src/coco_nao/error_rotation_right_6.txt");

    fileIndexJacob = Save.createFile("/home/cris/catkin_ws/src/coco_nao/jacobian_rotation_6.txt");
    fileIndexDelta = Save.createFile("/home/cris/catkin_ws/src/coco_nao/delta_rotation_6.txt");


    fileIndexInitial = Save.createFile("/home/cris/catkin_ws/src/coco_nao/initialjoints_rotation_6.txt");
    fileIndexFinal = Save.createFile("/home/cris/catkin_ws/src/coco_nao/joints_rotation_final_6.txt");
...
void cocoNAO::DQinvKin(const coco_nao::NaoJointPtr& joint_msg)
{
    ...
    RotationController controller(nao_chainR, nao_chainL, kp, t_damp); //controller constructor
    ...
    DQ rotEff = eff_pose_currentR.P().conj();

    //Difference Between the end effector position and the object

    eff_rotation_difference = vec4( rotEff - eff_pose_olderR); 
    DQ err_save = vec4( rotEff - eff_pose_olderR);
    DQ eff_rot_left = vec4( rotEff - eff_pose_olderL); 
    //cout << "rotEff " <<rotEff.translation() << endl << "x_rel " << ((eff_pose_currentR.P())*x_rel.translation())*eff_pose_currentR.P().conj() << endl << eff_rotation_difference << endl;

    
    if( (eff_rotation_difference.vec4().norm() > rotation_threshold) )
    {   
        //One controller step
        joints = controller.getNewJointPositions(rotEff, joints);
    ...
    VectorXd errorL = controller.error_rotation_L_;
    VectorXd errorR = controller.error_rotation_R_;    

    Save.savePosition(eff_pose_currentL, currentTime, fileIndexPosL);
    Save.saveError(errorL, currentTime, fileIndexErrorL);
    Save.savePosition(eff_pose_currentR, currentTime, fileIndexPosR);
    Save.saveError(errorR, currentTime, fileIndexErrorR);
