...
AgentEffector::AgentEffector() {
   ...
   qiLogVerbose("Motion") << "AgentEffector constructed" << std::endl;
}
...
AgentEffector::~AgentEffector() {
   ...
   qiLogVerbose("Motion") << "AgentEffector destroyed" << std::endl;
}
...
void AgentEffector::actuate(JointValues joints, ActionCommand::LED leds,
   ...
   //for(int j=0; j < Joints::NUMBER_OF_JOINTS; j++)
    //qiLogVerbose("Motion") << shared_data->joints[i].angles[j] << " " << shared_data->joints[i].stiffnesses[j] << std::endl;
