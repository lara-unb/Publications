...
HeadGenerator::HeadGenerator()
   : yaw(0.0f),
     pitch(0.0f) {
   qiLogVerbose("Motion") << "HeadGenerator constructed" << std::endl;
}

HeadGenerator::~HeadGenerator() {
   qiLogVerbose("Motion") << "HeadGenerator destroyed" << std::endl;
}
...
JointValues HeadGenerator::makeJoints(ActionCommand::All* request,
   ...
   if (isnan(request->head.yaw) || isnan(request->head.pitch)) {
      qiLogVerbose("Motion") << "HeadGenerator: NAN angles - Doing Nothing" << std::endl;
